  			...
  			<?php if (Salamander::getData('legacy_posts_slideshow')) : ?>
		          ...
		          <?php if(get_post_meta($post->ID, 'sl_meta_video', true)): ?>
			          ...
			          <?php if(has_post_thumbnail() && !get_post_meta($post->ID, 'sl_meta_video', true)): ?>
				          ...
				          ?>
			          <li>
			            <a href="<?php print $full_image[0]; ?>" rel="prettyPhoto[gallery<?php the_ID(); ?>]" title="<?php print get_post_field('post_content', get_post_thumbnail_id()); ?>"><img src="<?php print $attachment_image[0]; ?>" alt="<?php print get_post_field('post_excerpt', get_post_thumbnail_id()); ?>" /></a>
			          </li>
			          <?php endif; ?>
...
<!-- page-part.php -->
			...
			<aside id="right-sidebar" class="<?php print Salamander::classes('data', 'blog_sidebar_position', 'sidebar')?>"><?php if (!function_exists('dynamic_sidebar') || !dynamic_sidebar('Blog Right Sidebar')) : endif; ?></aside>
			<?php endif;?>
		</div>
	</div>
<?php get_footer(); ?>
